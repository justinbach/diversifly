<script type="text/template" id="palette-template">
  <!--<div id="palette-goal" class="goal"></div>-->
  <div id="palette" class="page-container">
    <img src="img/diversifly_logo.png" width="180" height="134" />
    <h1 class="banner">SELECT COLOR PALETTE</h1>
    <div class="spacer"></div>
    <div class="palette-page">
    </div>
    <div class="buttons">
      <a id="palette-prev-button" class="button nav red" href="#"><span>Previous Page</span></a>
      <a id="palette-next-button" class="button nav yellow" href="#"><span>Next Page</span></a>
    </div>
  </div>
</script>

<script type="text/template" id="butterfly-palette-template">
      <a href="butterfly/<%= id %>" class="butterfly-palette nav">
        <img src="img/palette.php?<%= colorString %>" width="224" height="157"/>
      </a>
</script>